...
export function useZiipEngine() {
  ...
  const loadFileSmart = useCallback(async (file) => {
    ...
    try {
      ...
      // Detect structure
      const structure = await detectCSVStructure(rawLines);
      ...
      if (structure.hasMultiRowHeader && structure.multiRowHeaderEnd) {
        ...
        const mergedHeaders = await mergeMultiRowHeaders(headerRows);
    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          sql = await generateSQL(question, context, lastError);
          setLastQuery(sql);
  ...
  const explainLastResults = useCallback(async (question) => {
    if (!lastQuery || !lastResults) return null;
    return explainResults(lastQuery, lastResults.rows, question);
  }, [lastQuery, lastResults]);
